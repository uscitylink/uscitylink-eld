...
# LOCAL FLEET MAP FROM ELD EVENTS
...
def save_samsara_gps():
    ...
    for truck in data:
        ...
        gps_time = truck.get("time")

        if not vehicle_id or not latitude or not longitude or not gps_time:
            skipped += 1
            continue
        ...
        existing = (
            ...
            .filter(GPSPoint.gps_time == gps_time)
            .first()
        )
...
def get_saved_gps_route(vehicle_id: str, limit: int = 500):
    db = SessionLocal()

    points = (
        db.query(GPSPoint)
        .filter(GPSPoint.vehicle_id == vehicle_id)
        .order_by(GPSPoint.gps_time.desc())
        .limit(limit)
        .all()
    )
...
def import_samsara_drivers():
    ...
    for d in data:
        samsara_id = d.get("driver_id")
        name = d.get("name") or "Unknown"
        license_number = d.get("license_number") or ""
        active = d.get("active") or "YES"
